...
var async = require('async'),
	...
	'https://b1f1f1bf6c6babf59bc957d38db4af7d33416ea5.cloudapp-enterprise-preprod.appctest.com' :
	...
	fn: function list(logger, config, cli, finished) {
		...
		async.parallel(tasks, function(err, results) {
			var defaultInstallLocation = cli.env.installPath,
				locations = cli.env.os.sdkPaths.map(function(p) {
			...
			if (cli.argv.output == 'json') {
				...
				var activeLabel = ' [' + __('selected') + ']',
					...
					maxname = vers.reduce(function(a, b) {
				...
				vers.forEach(function(v) {
					...
					logger.log('   ' + v.cyan + d.grey + new Array(n + 1).join(' ') + (maxname ? appc.string.rpad(name ? name :
						'', maxname + 2).magenta : '') + sdks[v].path);
				});
				logger.log();
				...
				if (results.releases) {
					...
					if (results.releases instanceof Error) {
						...
						Object.keys(results.releases).sort().reverse().forEach(function(r) {
							logger.log('   ' + r.cyan + (sdks.hasOwnProperty(r) ? ' [' + __('installed') + ']' : '') + (i++ == 0 ?
								' [' + __('latest') + ']' : ''));
						});
					...
					if (branchBuilds) {
						...
						if (branchBuilds.length) {
							...
							branchBuilds.forEach(function(f) {
								...
								//	}
								logger.log('   ' + (f.version + '.v' + f.ts).cyan + '  ' + appc.string.rpad(f.dateFormatted, maxdate) +
...
SdkSubcommands.select = {
	...
	fn: function select(logger, config, cli, finished) {
		var selectedSDK = cli.argv.version,
			...
			activeSDK = config.get('sdk.selected', config.get('app.sdk', 'latest')) == 'latest' && vers.length ? vers[0] :
			config.get('sdk.selected', config.get('app.sdk')),
...
SdkSubcommands.install = {
	...
	fn: function install(logger, config, cli, finished) {
			...
			getReleases(config, osName, function(err, releases) {
				...
				if (releases[version]) {
					...
					scanBranches(version, function(err) {
						...
						logger.log(__('Found build %s in branch %s', version.cyan, branch.cyan) + '\n');
		...
		function doBranch(version, callback) {
			...
			fetch(urls.branches, config, function(err, data) {
				...
				getBranchBuilds(config, branch, osName, function(err, builds) {
					...
					var isLatest = !version || version == 'latest',
						build = isLatest ? builds[0] : builds.filter(function(b) {
							return b.name == version;
						}).shift();

					if (!build) {
						var buildNames = builds.map(function(b) {
							return b.name;
						}).sort().reverse();
					...
					if (!cli.argv.force && cli.env.sdks[version]) {
						...
						logger.log(__("Run '%s' to re-install.", (cli.argv.$ + ' sdk install ' + version + ' --force --branch ' +
							branch).cyan) + '\n');
						process.exit(0);
					}

					callback(null, {
						url: urls.build.replace(/\$BRANCH/, branch).replace(/\$FILENAME/, build.filename).replace(/\$TOKEN/,
							process.env.APPC_SESSION_TOKEN || 'X'),
						version: version,
						branch: branch
					});
				});
		...
		function extractSDK(filename, keepFiles, version, callback) {
			...
			try {
				var manifest = JSON.parse(appc.zip.readAsText(filename, 'ZIP_META_MANIFEST'));
				var name = manifest.name;
				var platform = manifest.platform;
				if (name && platform) {
					logger.log(__('Removing existint SDK at %s', path.join(installLocation, 'mobilesdk', platform, name).cyan));
					afs.unlinkDirSyncRecursive(path.join(installLocation, 'mobilesdk', platform, name));
				}
			} catch(e) {
			...
			unzipper.on('progress', function (fileIndex, fileCount) {
				if (!bar && !cli.argv.quiet && !!cli.argv['progress-bars']) {
			...
			unzipper.extract({
				...
				filter: function(file) {
					if (/ZIP_META_MANIFEST/.test(file.path)) {
						return false;
					}
					return true;
				}
		...
		async.series([
			function(next) {
				if (version) {
					...
					} else if (/.+\.zip$/.test(version) && fs.existsSync(versionPath = afs.resolvePath(version)) && fs.statSync(
							versionPath).isFile()) {
						// version is a file
						next(null, {
							file: versionPath
			...
			var vers = Object.keys(cli.env.sdks).filter(function(v) {
				...
				newestInstalledSdk = config.get('sdk.selected', config.get('app.sdk', 'latest')) == 'latest' ? (vers.length ?
					vers[0] : null) : config.get('sdk.selected', config.get('app.sdk')),
				setDefault = (data.setDefault || cli.argv.default) && appc.version.gte(data.version, '3.0.0') && (cli.argv.default ||
					newestInstalledSdk === null || appc.version.gt(data.version, newestInstalledSdk)),
				analyticsPayload = {};
...
function fetch(url, config, callback) {
	request({
		...
		if (error) {
			...
			callback(new Error(__('Request failed with HTTP status code %s %s', response.statusCode, http.STATUS_CODES[
...
function getReleases(config, os, callback) {
	fetch(urls.releases, config, function(err, data) {
		...
		callback && callback(err, err ? new Error(__('Failed to get releases: %s', err.message || err.toString())) :
			releases);
	});
}
...
function getBranches(config, callback) {
	fetch(urls.branches, config, function(err, branches) {
		callback && callback(err, err ? new Error(__('Failed to get branches: %s', err.message || err.toString())) :
			branches);
	});
}
...
function getBranchBuilds(config, branch, osName, callback) {
	fetch(urls.branch.replace(/\$BRANCH/, branch).replace(/\$TOKEN/, process.env.APPC_SESSION_TOKEN || 'X'), config,
		function(err, builds) {
			if (!err && builds) {
